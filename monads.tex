...
\todo{Regarding the epigraph, should it be mentioned that it seems
  like monads are everywhere? Music, religion, philosophy, ...}
...
belongs to \texthaskell{ys}, that is,
\begin{equation*}
...
\begin{examples}
  [\footnote{These examples are based on \parencite[58]{moggi-1991}.}]
  ...
  \item
    \emph{Partiality}
  \item
    \emph{Nondeterminism} \todo{Mention Section
      \ref{sec:monads-introduction}.}
  \item
    \emph{Side effects}
  \item
    \emph{Exceptions}
...
\begin{definition}
  [Monad\footnote{This definition is based on
...
\todo{When referring to a term, choose between `monad' and \term{monad}.}

\begin{terminology}
  \label{ter:monad}
  Although the common term for monads is \term{monad}, alternatives
  include \term{standard construction}, which is the original term
  \parencite[30]{manes-1976}, \term{algebraic theory in monoid form}
  \parencite[29]{manes-1976}, and \term{triple}
  \parencites[83]{barr-2005}[372]{barr-wells-2012}.
  %% , which seems to be the only current alternative.
  We choose \term{monad} because of Remark
  \ref{re:monad-monoid} and reserve \term{triple} for effectively
  distinguishing between monads and triples, as explained in page
  \pageref{ter:triple}.
\end{terminology}

\todo{If more examples are added, consider introducing the examples.}
...
\begin{definition}
  [Kleisli triple\footnote{This definition is based on
      \parencite[58]{moggi-1991}.}]

  \label{def:triple}

  \index{triple}
  \index{Kleisli triple|see{triple}}
  \index{triple!Kleisli ---|see{triple}}

  \index{monad!--- in extension form}
...
\begin{terminology}
  \label{ter:triple}
  The common term for Kleisli triples is \term{Kleisli triple}. Yet
  another term is \term{algebraic theory in extension form}
  ...
  something like \term{monad in extension form}, we choose
  \term{Kleisli triple} for the sake of simplicity. When defining
  monads, \textcite[138]{maclane-1998} considers the use of
  \term{triple} as ``frequent but unfortunate,'' but this is not the
...
\begin{examples}
  [Notions of computation\footnote{These examples are based on
...
\todo{Introduce Example \ref{ex:monad-triple-identity}.}
...
instance Monad' Maybe where
  ...
  \begin{proof}
    ...
    \begin{steps}
      ...
      \steph{join Nothing}
        \eqbydef{\texthaskell{fmap} \eqref{eq:maybe-fmap-nothing}}
    ...
    \begin{steps}
      ...
      \steph{join (Just (join mmx))}
        \eqbydef{\texthaskell{fmap} \eqref{eq:maybe-fmap-just}}
