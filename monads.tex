...
\todo{Unlike Examples \ref{ex:monad-identity} and
...
\begin{example}
  ...
  defined as follows\footnote{Using the \texthaskell{InstanceSigs}
    language option.}:
  \begin{codehaskell}
...
As examples, we consider the identity, \texthaskell{Maybe}, and list
...
\begin{example}
  ...
  In \hask, the \texthaskell{Maybe} Kleisli triple, which corresponds
  to the standard \texthaskell{Maybe} monad, is defined as follows:
  \begin{codehaskell}
instance Monad Maybe where
  return :: a -> Maybe a
  return = Just
...
\begin{remark}
  ...
  monads without \texthaskell{return}). The type class
