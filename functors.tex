...
map :: (a -> b) -> [a] -> [b]
\end{codehaskell}
According to the documentation of the Haskell Prelude\todo{Reference}, given a
...
\begin{definition}
  ...
  Let \cat{C} and \cat{D} be categories\footnote{\todo{In the
      sequel?}Whenever a category \cat{C} is mentioned, its sets of
    objects and morphisms, its domain and codomain functions, and its
    identity and composition functions (\catO{C} and \catM{C}, $\dom$
...
\begin{example}
  ...
  Given a category \cat{C}, its identity \todo{endofunctor} $\func{I}:
