...
\begin{definition}
  ...
  \item
    ...
    A set of objects \catO{C}.

  \item
    \index{morphism}
    \index{arrow|see{morphism}}

    A set of morphisms or arrows \catM{C}.
...
\begin{example}
  ...
  \cat{C} in which $\catO{C} = A$ and $\catM{C} = \{\idO{a}: a \in
...
\begin{example}
  ...
  Likewise, \catbf{Par} is the category of small sets and partial
  functions, and \catbf{Rel} is the category of small sets and
  relations. Note that, while category-theorists usually name
  categories after the common name of their objects---for instance,
  \set---, \catbf{Par} and \catbf{Rel} are named after the common
  names of their morphisms, which suggests emphasizing morphisms. In
  fact, \textcquote[30]{maclane-1998}{category theory asks of every
    type of mathematical object: \enquote{What are the morphisms?}}.
...
\begin{example}
  ...
  and monoid homomorphisms) yield a category, \catbf{Mon}. This
...
\begin{example}
  ...
  A preorder \cat{P}, also known as an order category, is a category
  in which, for all objects $a$ and $b \in \catO{P}$, there is at most
  one morphism $f: a \to b \in \catM{P}$. Given a preorder \cat{P}, we
  define a binary relation $\leq$ on \catO{P} with $a \leq b$ if and
  ...
  relation $\leq$ on $P$, we get a category \cat{P} with $\catO{P} =
  ...
  c$ for all $a$, $b$, and $c \in P$). \catbf{Pre} is the category of
...
expressions with kind \texthaskell{*} such as \texthaskell{Bool}:
\begin{codehaskell}
