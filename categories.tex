...
\texthaskell{Suc}, \texthaskell{isZero}, and \texthaskell{not}, the
...
\begin{figure}[htb]
  \begin{center}
    \begin{tikzpicture}[node distance=4cm]
      ...
      \draw [->] (nat)  to [loop above] node        {\texthaskell{Suc}}  (nat);
...
instance, that \texthaskell{id . Suc = Suc = Suc . id}, which
...
\begin{example}
  ...
  the types \texthaskell{()}, \texthaskell{Bool}, and
  \texthaskell{Nat}, and its morphisms are the constants-as-functions
  \begin{itemize}
  \item
    \texthaskell{() :: () -> ()},
  ...
  \item
    \texthaskell{Suc . Zero}, \texthaskell{Suc . (Suc . Zero) :: () -> Nat}, ...,

  \item
    \texthaskell{isZero . Zero}, \texthaskell{isZero . (Suc . Zero) :: () -> Bool}, ...,
...
\texthaskell{*}. For instance, the Boolean type, \texthaskell{Bool}:
\begin{codehaskell}
...
\texthaskell{Zero} and \texthaskell{Suc Zero}, but also
...
or \textagda{Set}, which is called the universe of small types, will
be the only universe necessary for our development (\todo{From Andrés'
  thesis}).
...
And, for each pair of functions \textagda{f : A → B} and \textagda{g:
  B → C}, there is a composite function defined as follows,
\textagda{g ∘ f}:
\begin{codeagda}
infixr 9 _∘_
