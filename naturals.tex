...
the first element of a list\footnote{Note that this is not the
  standard \texthaskell{head} function.}:
\begin{codehaskell}
head :: [a] -> Maybe a
head []    = Nothing
head (x:_) = Just x
...
\texthaskell{Maybe} functor. Besides, it is not just a function, but
...
\begin{example}
  ...
  Given a category \cat{C}, its identity function is a natural
...
functors with type constructors \texthaskell{f} and \texthaskell{g}, a
natural transformation \texthaskell{tau} is a polymorphic function
\begin{codehaskell}
tau :: f a -> g a
\end{codehaskell}
Or, using the \texthaskell{ExplicitForAll} language option, a set of
functions indexed by Haskell types:
\begin{codehaskell}
tau :: forall a. f a -> g a
\end{codehaskell}
The naturality of \texthaskell{tau} is the commutativity of the
diagram in Figure \ref{fig:naturality-haskell}, that is, for all
functions \texthaskell{f :: a -> b}:
\begin{codehaskell}
...
\begin{example}
  ...
  \texthaskell{Maybe} functor. Its naturality is the commutativity of
...
\begin{example}
  ...
  \texthaskell{Maybe} functor is the \texthaskell{last} function,
  which extracts the last element of a list\footnote{Note that this is
    not the standard \texthaskell{last} function.}:
  \begin{codehaskell}
last :: [a] -> Maybe a
last []     = Nothing
last (x:[]) = Just x
...
last . fmap f = fmap f . last
  ...
  \begin{figure}[htb]
    \begin{center}
      ...
      \end{tikzpicture}
    \end{center}
    \caption{Naturality of the \texthaskell{last} function in Haskell.}
    \label{fig:naturality-last-haskell}
  \end{figure}

  \begin{proof}

    We prove the naturality of the \texthaskell{last} function by
    ...
    \begin{steps}
      ...
      \steph{(fmap f . last) (x:[])}
    \end{steps}
    And in the case of a nonempty list \texthaskell{(x:y:ys)}:
    \begin{steps}
